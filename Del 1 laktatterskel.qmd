...
modlaktatterskel <- lm(lactate ~ poly(watt, 3, raw = TRUE), data = Laktatterskel)
...
wattdata$pred <- predict(modlaktatterskel, newdata = wattdata)
...
modlaktatterskel.meso1 <- lm(lactate ~ poly(watt, 3, raw = TRUE), data = Laktatterskel.meso1)
...
wattdata.meso1$pred <- predict(modlaktatterskel, newdata = wattdata.meso1)

lactate.meso1 <- wattdata.meso1 %>% 
  filter(abs(pred - 2) == min(abs(pred - 2)) |
           abs(pred - 4) == min(abs(pred - 4))) %>% 
  mutate(timepoint = "medso1",
         lactate = "2mmol")
...
modlaktatterskel.meso2 <- lm(lactate ~ poly(watt, 3, raw = TRUE), data = Laktatterskel.meso2)
...
wattdata.meso2$pred <- predict(modlaktatterskel, newdata = wattdata.meso2)
...
modlaktatterskel.meso3 <- lm(lactate ~ poly(watt, 3, raw = TRUE), data = Laktatterskel.meso3)
...
wattdata.meso3$pred <- predict(modlaktatterskel, newdata = wattdata.meso3)
